...
def update_status(id):
    """Update board status — prevent reverting Completed → Pending."""
    status = request.form['board_status']
    now = datetime.now().strftime("%Y-%m-%d %I:%M %p")

    conn = sqlite3.connect(LOCAL_DB)
    c = conn.cursor()
    c.execute("SELECT board_status FROM rework_tracker WHERE id=?", (id,))
    ...
    # 🛑 Prevent revert
    if current_status == "Rework Completed":
        conn.close()
        print(f"⚠️ Prevented revert for ID {id}")
        return redirect('/')
...
#pyinstaller --onefile --noconsole --icon=Solderingiron.ico --name "ReworkTracker" --hidden-import=sqlite3 --add-data "templates;templates" --add-data "static;static" app.py